...
export class UrlLengthTester extends Component {
  ...
  render() {
    let contents =
      (!!this.state.returnedData && <div><h4>Server-side view:</h4><div>{this.state.returnedData}</div></div>) ||
      (!!this.state.returnedError && <div><h4>Error:</h4><div>{this.state.returnedError}</div></div>);
